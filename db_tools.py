...
#Calculate website availability (available = 2xx Status Code), avarage response time, total checks and current status
import sqlite3
from datetime import datetime, timezone
...
def get_website_availability_stats(url):
    ...
    if not row:
        ...
        success_count = sum(1 for log in logs if str(log[0]).startswith('2'))
    ...
    return success_ratio, website_name, average_response_time, current_status
